...
# Count cases per date for tooltips
...
# Create unique tooltip per date (one row per date in column mode)
large_outbreak$tooltip <- with(large_outbreak, paste0(
  "<b>Date:</b> ", format(onset_date, "%d %b %Y"), "<br>",
  "<b>Cases:</b> ", count
))

# Get one row per date for the plot
plot_data <- large_outbreak[!duplicated(large_outbreak$onset_date), ]

p <- ggplot(plot_data, aes(x = onset_date, text = tooltip)) +
...
Add context to your epidemic curves with annotations. **Note:**
Annotations are visible in the static ggplot but may not appear in the
interactive plotly conversion. For fully interactive timelines, consider
adding vertical lines as geoms instead:

``` r

# This works great for static plots, less so for plotly
...
# For interactive (annotations may not convert)
# ggplotly(p)
```
