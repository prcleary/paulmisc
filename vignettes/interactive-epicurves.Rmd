...
Epidemic curves work with different time scales. Here is a richer
hourly outbreak with a continuous source over three days &mdash; this
produces a curve whose bars vary in height, with proper time labels on
the x-axis:
...
For large outbreaks, the plot automatically switches to column chart mode when case counts exceed the threshold. By default `geom_epicurve()` builds a sensible tooltip per case &mdash; date plus case count for that day &mdash; so you don't need to pre-aggregate the data yourself:
...
to categories automatically, and the legend updates without needing
any `guides()` boilerplate:
...
Timeline annotations work seamlessly with plotly using the same code as for static plots:
...
The shaded period, event line, and all labels convert to plotly automatically!
